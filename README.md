...
- OneScript (oscript) - для выполнения скриптов. Ссылка [https://oscript.io/](https://oscript.io/)
...
Примеры файлов packagedef + env.json + oscript_modules в репозитории [https://github.com/asweetand-a11y/DevTool1C](https://github.com/asweetand-a11y/DevTool1C).
...
В папку с проектом скопировать oscript_modules для подмены модулей onescript. Оригиналы можно взять [https://github.com/asweetand-a11y/DevTool1C](https://github.com/asweetand-a11y/DevTool1C).
...
### Отладка модулей расширений

`rootProject` в launch/attach — корень выгрузки (каталог с `src/cf` и `src/cfe`). Пример: основная конфигурация `D:/DATA1C/BASE/src/cf`, расширение Расш1 — `D:/DATA1C/BASE/src/cfe/Расш1`.

### Доступ агента Cursor к текущей отладке
...
2. MCP: `extension-1c-debug`:
  ...
  - Шаги: `onec_debug_step_over` (F10), `onec_debug_step_into` (F11), `onec_debug_step_out` (Shift+F11), `onec_debug_continue` (F5). После шага возвращается обновлённый стек.
  - Чат — в том же окне, что сессия `onec`. Типичный сценарий агента: `onec_debug_set_breakpoint` → `onec_debug_start` → дождаться останова (`onec_debug_stack`) → шаги и Watch. Сам пользовательский сценарий в клиенте 1С (кнопка, проведение документа) агент нажать не может — после старта отладки действие в Предприятии нужно выполнить вручную или дождаться автозапуска `launch`.
