...
Процедура ЗагрузитьКонфигурациюИзФайлов(Знач КаталогЗагрузки,
                                        ...
                                        Знач ПутьКФайлуЛогаЗагрузки = "") Экспорт
	...
	Если ЗначениеЗаполнено(ПутьКФайлуЛогаЗагрузки) Тогда
		Параметры.Добавить("/Out " + ОбернутьВКавычки(ПутьКФайлуЛогаЗагрузки));
	КонецЕсли;
...
// Загрузка внешней обработки или отчета из файлов (/LoadExternalDataProcessorOrReportFromFiles)
//
// Параметры:
//   ПутьККаталогуВыгрузки - Строка - Корневой XML (например ExternalDataProcessor.xml) или каталог исходников;
//       платформа ожидает в командной строке путь к файлу; для каталога корневой XML ищется автоматически.
//   ПутьКВнешнейОбработкеИлиОтчету - Строка - Путь к целевому файлу .epf / .erf
//
Функция НайтиКорневойФайлВыгрузкиВнешнейОбработкиИлиОтчета(Знач КаталогВыгрузки)

	ИменаКорневыхФайлов = Новый Массив;
	ИменаКорневыхФайлов.Добавить("ExternalDataProcessor.xml");
	ИменаКорневыхФайлов.Добавить("ExternalReport.xml");

	Для Каждого ИмяФайла Из ИменаКорневыхФайлов Цикл
		ПутьКФайлу = ОбъединитьПути(КаталогВыгрузки, ИмяФайла);
	...
	ВызватьИсключение "В каталоге """ + КаталогВыгрузки + """ не найден корневой файл выгрузки (ExternalDataProcessor.xml или ExternalReport.xml). Укажите каталог исходников или путь к корневому XML.";

КонецФункции

Процедура ЗагрузитьВнешниеОтчетыИлиОбработкиИзФайлов(Знач ПутьККаталогуВыгрузки,
	Знач ПутьКВнешнейОбработкеИлиОтчету,
	Знач Формат = "") Экспорт
	...
	ФайлВыгрузки = Новый Файл(ПутьККаталогуВыгрузки);
	ПолныйПутьВыгрузки = ФайлВыгрузки.ПолноеИмя;

	Если ФайлВыгрузки.ЭтоКаталог() Тогда
		ОбеспечитьКаталог(ПолныйПутьВыгрузки);
		КорневойФайлВыгрузки = НайтиКорневойФайлВыгрузкиВнешнейОбработкиИлиОтчета(ПолныйПутьВыгрузки);
	Иначе
		Если НЕ ФайлВыгрузки.Существует() Тогда
			ВызватьИсключение "Не найден файл или каталог выгрузки внешней обработки/отчета: " + ПолныйПутьВыгрузки;
		КонецЕсли;
		КорневойФайлВыгрузки = ПолныйПутьВыгрузки;
	КонецЕсли;
	...
	ПараметрыЗапуска.Добавить(ОбернутьВКавычки(КорневойФайлВыгрузки));
	ПараметрыЗапуска.Добавить(ОбернутьВКавычки(ПутьКВнешнейОбработкеИлиОтчету));
...
Процедура ЗапуститьВРежимеКонфигуратора(Знач КлючЗапуска = "", Знач УправляемыйРежим = Неопределено, Знач ДополнительныеКлючи = Неопределено) Экспорт
	ПараметрыСвязиСБазой = ПолучитьПараметрыЗапуска();
