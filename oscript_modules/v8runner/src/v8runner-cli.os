...
Процедура ВывестиСправку()
	...
	Сообщить("  --src <путь>             - Корневой XML выгрузки (ExternalDataProcessor.xml / ExternalReport.xml) или каталог исходников");
...
Процедура ВыполнитьLoadConfigFromFiles()
	...
	ПутьКФайлуЛога = ПолучитьПараметр("--out");
	...
	Если Не ПустаяСтрока(ПутьКФайлуЛога) Тогда
		Сообщить("  Лог Конфигуратора: " + ПутьКФайлуЛога);
	КонецЕсли;
...
Процедура ВыполнитьLoadExternalFiles()
	...
	ПутьККорневомуФайлуВыгрузки = ПолучитьПараметр("--src");
	ПутьКВнешнейОбработкеИлиОтчету = ПолучитьПараметр("--file");
	Формат = ПолучитьПараметр("--format");
	
	Если ПустаяСтрока(ПутьККорневомуФайлуВыгрузки) Тогда
		ВызватьИсключение "Не указан обязательный параметр --src (корневой XML выгрузки или каталог исходников)";
	КонецЕсли;
	...
	Сообщить("  Корневой файл/каталог выгрузки: " + ПутьККорневомуФайлуВыгрузки);
	Сообщить("  Целевой .epf/.erf: " + ПутьКВнешнейОбработкеИлиОтчету);
